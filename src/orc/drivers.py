...
class DriverBase(eqx.Module, ABC):
    ...
    def __init__(self, res_dim, dtype=jnp.float64):
        """Ensures reservoir dim and dtype are correct type."""
...
class ESNDriver(DriverBase):
    ...
    def __init__(self,
        ...
        wr_indices = jax.random.choice(wrkey1,
                                      res_dim ** 2,
                                      shape=(N_nonzero,),
                                      )
        wr_vals = jax.random.uniform(wrkey2,
                                    shape=N_nonzero,
                                    minval=-1,
                                    maxval=1,
                                    dtype=self.dtype
    ...
    def advance(self,
                proj_vars: Float[Array, "{self.res_dim}"],
        ...
        """
        res_next = jnp.tanh(self.wr @ res_state
                            + proj_vars
                            + self.bias * jnp.ones(self.res_dim))
